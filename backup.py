...
# Certifique-se de ter importado sqlite3 e get_db_connection() se ainda não o fez.
# Por exemplo, se get_db_connection() está em modules.user:
# from modules.user import get_db_connection
...
def relatorio_opcoes():
    ...
    # 1. Obter o nome de usuário da sessão
    username = session['user'] # Use 'user' ou 'username' dependendo do que você usa na sessão

    # 2. Conectar ao banco de dados para buscar o e-mail do usuário
    conn = None # Inicialize conn fora do try para garantir que esteja definido
    usuario_email = None
    try:
        conn = sqlite3.connect('database/finflow.db') # Certifique-se que esta função está definida e funcional
        cursor = conn.cursor()
        
        # Adapte esta consulta para sua tabela de usuários e nome das colunas
        cursor.execute("SELECT email FROM users WHERE username = ?", (username,))
        user_data = cursor.fetchone() # user_data será uma tupla ou None
        
        if user_data:
            usuario_email = user_data['email'] # Se você usar row_factory = sqlite3.Row
            # Se não usar row_factory, seria: usuario_email = user_data[0]
        else:
            print(f"AVISO: E-mail não encontrado para o usuário: {username}")
            # Você pode querer adicionar um flash message ou logar isso
            
    except Exception as e:
        print(f"Erro ao buscar e-mail do usuário no DB: {e}")
        # Lidar com o erro de conexão/consulta, talvez definir um email padrão ou None
    finally:
        if conn:
            conn.close()

    # Crie um dicionário para passar ao template, simulando um objeto 'usuario'
    # Mesmo que o e-mail seja None, isso evita o UndefinedError
    usuario = {'email': usuario_email if usuario_email else ''} 
    
    # 3. Passar a variável 'usuario' para o template
...
# Certifique-se de que todas as importações necessárias para gerar PDF (reportlab, BytesIO, etc.)
# e para listar empréstimos (listar_emprestimos_ativos, etc.) estejam disponíveis neste arquivo.
# Já vimos que elas estão presentes nas outras funções de relatório.

@app.route('/gerar_relatorio_personalizado', methods=['POST'])
def gerar_relatorio_personalizado():
    """Gera um relatório PDF personalizado com base nos filtros selecionados"""
    if 'user' not in session or session.get('tipo') != 'emprestador':
    ...
    # Seção de empréstimos vencendo (apenas para relatório completo)
    ...
    # Função auxiliar para formatar os dados de cada empréstimo
    def formatar_emprestimo(emp, tipo='emprestimo'):
        emp_dict = dict(emp)  # 👈 Converte para dict
        partes = []
    ...
    # Adicionar cada seção ao relatório
    secoes = [
...
def enviar_relatorio_email():
    ...
    try:
        # --- Gerar o PDF em memória (CÓDIGO COPIADO E ADAPTADO DA FUNÇÃO gerar_pdf) ---
        buffer = BytesIO()
        doc = SimpleDocTemplate(buffer, pagesize=letter)
        styles = getSampleStyleSheet() # Corrigido o erro de digitação aqui também
        story = []
        ...
        for emp in ativos:
            emp_dict = dict(emp) # Converte sqlite3.Row para dicionário
            texto = (
        ...
        # Configurar e-mail
        msg = MIMEMultipart()
        msg['From'] = remetente_email # <--- USANDO A VARIÁVEL DE AMBIENTE AQUI
        ...
        # Enviar e-mail
        # AGORA USANDO O SERVIDOR CORRETO DO GMAIL E AS VARIÁVEIS DE AMBIENTE PARA LOGIN
        with smtplib.SMTP('smtp.gmail.com', 587) as server: # <--- SERVIDOR GMAIL AQUI!
            server.starttls()
            server.login(remetente_email, senha_app_email) # <--- USANDO AS VARIÁVEIS DE AMBIENTE AQUI
...
def gerar_pdf():
    """
    Gera um relatório PDF simples contendo empréstimos ativos e finalizados.
    Retorna o PDF para download direto no navegador.
    """
    # Verifica a autenticação e o tipo de usuário
    if 'user' not in session or session.get('tipo') != 'emprestador':
        flash("Acesso negado. Faça login como Emprestador.", "error")
        return redirect(url_for('login'))
    ...
    if not ativos: # Mensagem se não houver empréstimos ativos
        story.append(Paragraph("Nenhum empréstimo ativo encontrado.", styles['Normal']))
    for emp in ativos:
        # Acessa os dados como dicionário, já que listar_emprestimos_ativos deve retornar sqlite3.Row
        texto = (
    ...
    doc.build(story) # Constrói o documento PDF
    buffer.seek(0) # Volta ao início do buffer para ler o conteúdo
