...
def listar_solicitacoes_pendentes(username):
    """
    Retorna uma lista de solicitações com status 'pendente' para um emprestador específico.
    """
    conn = sqlite3.connect('database/finflow.db')
    conn.row_factory = sqlite3.Row  # isso permite acessar dados como dicionário
    solicitacoes = []
    try:
        cursor = conn.cursor()
        # Seleciona todas as colunas da tabela de solicitacoes onde o emprestador corresponde
        # e o status é 'pendente'.
        cursor.execute("SELECT * FROM solicitacoes WHERE emprestador = ? AND status = 'pendente'", (username,))
...
# Função para listar todos os empréstimos de um usuário, incluindo métricas
# Retorna um dicionário com listas de empréstimos ativos, finalizados, pendentes
def listar_todos_emprestimos(username):
    """Lista todos os empréstimos de um usuário, incluindo métricas"""
    conn = sqlite3.connect('database/finflow.db')
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
